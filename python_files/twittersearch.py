...
#initialize db connection and specify db as twitter db
...
#if .twittersearch exists from earlier search, open is and set maxid
try:
    with open('.twittersearch', 'r') as conf:
        lines = conf.readlines()
        maxid = int(lines[0])
except:
    # else set maxid infinitive
    maxid = float('inf')
loop = 0

try:
    while True:
        while loop<5000: #do 5000 requests at a time, rest for 15 ms
            try:
                if maxid != float('inf'):
