...
var foc = (d3.min(array, function(d) { return d.x; }) + d3.max(array, function(d) { return d.x; }))/2;
  var xFisheye = d3.fisheye.scale(d3.scale.identity).domain([d3.min(array, function(d) { return d.x; }), d3.max(array, function(d) { return d.x; })]).focus(foc + 300/600/(d3.max(array, function(d) { return d.x; }) - d3.min(array, function(d) { return d.x; })));
...
function drawGraph(svgId, dataSet) {
    ...
    lines = svgContainer.select("g").selectAll(".line")
...
d3js(5);
drawGraph("#svg", array);
//drawGraph("#svg", instaArray);

drawGraph("#test", array);

drawGraph("#test2", array);

drawGraph("#test3", array);

drawGraph("#test4", array);

drawGraph("#test5", array);

drawGraph("#test6", array);
...
(function svg() {
 // console.clear()
  ...
  svg.on("mousemove", function() {
    ...
    var value = 600/(d3.max(array, function(d) { return d.x; }) - d3.min(array, function(d) { return d.x; }));
    xScale(xFisheye.focus(mouse[0]/value+d3.min(array, function(d) { return d.x; })));
...
function updateFishEye(m) {
    var lines = d3.selectAll("svg").select("g")
      .selectAll(".line");
